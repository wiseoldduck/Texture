...
// This is set by each test case to enable or disable the use of Yoga2. All tests are run twice,
// once with this flag set to NO and once with YES.
static BOOL gUseYoga2 = NO;
...
- (ASDisplayNode *)node {
  ...
  [_node.style yogaNodeCreateIfNeeded];
...
@property(nonatomic) BOOL useYoga2;
@end
...
+ (XCTestSuite *)defaultTestSuite {
  ...
  // Also run all tests with useYoga2 set to YES.
...
{
  // TODO(b/127807845): ASSizeRangeUnconstrained does not work, so we have to use CGFLOAT_MAX
...
{
  // TODO(b/127711064): Yoga is producing a different layout from Chrome.
  [root layoutIfNeededWithFrame:CGRectMake(0, 0, 301, 207)];
  [self assertFrame:CGRectMake(0, 0, 301, 207) forName:@"root" layout:root];
  [self assertFrame:CGRectMake(1, 7, 40, 30) forName:@"c0" layout:root];
